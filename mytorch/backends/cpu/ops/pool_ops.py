...
def cpu_max_pool2d(input_tensor: Tensor, kernel_size: tuple[int, int], stride: tuple[int, int],
    ...
    output = np.zeros((batch_size, channels, out_height, out_width)).astype(input_tensor.dtype.np_dtype)
            ...
            for c in range(channels):
                for h in range(out_height):
                    for w in range(out_width):
                        ...
                        output[b, c, h, w] = np.max(padded_input[b, c, h_start:h_end, w_start:w_end])
...
def cpu_max_pool2d_backward(output_grad_tensor: Tensor, output_tensor: Tensor, input_tensor: Tensor,
    ...
    @numba.njit
    def loop(output_grad, input_grad, output):
        for b in range(batch_size):
            for c in range(in_channels):
                for oh in range(output_height):
                    for ow in range(output_width):
                        ...
                        ih_end = min(ih_start + kernel_height, input_height + pad_height)
                        ...
                        input_grad[b, c, ih_start:ih_end, iw_start:iw_end] += mask * output_grad[b, c, oh, ow]

    loop(output_grad, input_grad, output)
