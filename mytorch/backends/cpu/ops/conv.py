...
def _im2col(
    ...
    for y in range(kernel_size[0]):
        ...
        for x in range(kernel_size[1]):
            ...
            col[:, :, y, x, :, :] = im[
                :, :, y : y_max : stride[0], x : x_max : stride[1]
            ]
...
def _col2im(
    ...
    col = col.reshape(
        batch_size, output_height, output_width, channels, *kernel_size
    ...
    im = np.zeros(
        (
            batch_size,
            channels,
            height + 2 * padding[0] + stride[0] - 1,
            width + 2 * padding[1] + stride[1] - 1,
        )
    )
    for y in range(kernel_size[0]):
        y_max = y + stride[0] * output_height
        for x in range(kernel_size[1]):
            x_max = x + stride[1] * output_width
            im[:, :, y : y_max : stride[0], x : x_max : stride[1]] += col[
                :, :, y, x, :, :
            ]
...
def cpu_conv2d(
    ...
    output_np_array = np.transpose(
        output_np_array.reshape(batch_size, output_height, output_width, -1),
        (0, 3, 1, 2),
    )
...
def cpu_conv2d_backward(
    ...
    # [C_out, C_in * k^2]
    output_grad_np_array = np.transpose(output_grad._numpy(), (0, 2, 3, 1)).reshape(
        (-1, output_grad.shape[1])
    )
    # [bhw, C_out]
    input_grad_np_array = np.matmul(output_grad_np_array, weight_np_array).astype(
        input.dtype.np_dtype
    )
    input_grad_np_array = _col2im(
        input_grad_np_array, input.shape, weight.shape[2:], stride, padding
    )
    input_grad = Tensor(input_grad_np_array, device=input.device)
    weight_grad_np_array = np.matmul(output_grad_np_array.T, input_np_array).astype(
        weight.dtype.np_dtype
    )
