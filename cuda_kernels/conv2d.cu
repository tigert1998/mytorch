...
template <typename T>
__global__ void conv2d_backward_reference(
    int batch_size, int input_h, int input_w, int in_channels, int out_channels,
    int kernel_size_h, int kernel_size_w, int stride_h, int stride_w,
    int padding_h, int padding_w, T *input, T *weight, T *bias, T *input_grad,
  ...
  for (int i = 0; i < kernel_size_h; i++)
    for (int j = 0; j < kernel_size_w; j++)
      for (int k = 0; k < in_channels; k++) {
        int weight_idx =
            ((out_c * in_channels + k) * kernel_size_h + i) * kernel_size_w + j;
        T weight_value = weight[weight_idx];
        int input_x = yid * stride_h + i - padding_h;
        int input_y = zid * stride_w + j - padding_w;
        bool is_padding = input_x < 0 || input_y < 0 || input_x >= input_h ||
                          input_y >= input_w;
        int input_idx =
            ((batch_idx * in_channels + k) * input_h + input_x) * input_w +
            input_y;
        T input_value = is_padding ? (T)0 : input[input_idx];
        if (!is_padding) {
          atomicAdd(&input_grad[input_idx],
                    output_grad[output_idx] * weight_value);
        }
        atomicAdd(&weight_grad[weight_idx],
                  output_grad[output_idx] * input_value);
      }
...
extern "C" __global__ void conv2d_backward_reference_fp32(
    int batch_size, int input_h, int input_w, int in_channels, int out_channels,
    int kernel_size_h, int kernel_size_w, int stride_h, int stride_w,
    int padding_h, int padding_w, float *input, float *weight, float *bias,
    float *input_grad, float *weight_grad, float *bias_grad,
    float *output_grad) {
  return conv2d_backward_reference<float>(
      batch_size, input_h, input_w, in_channels, out_channels, kernel_size_h,
      kernel_size_w, stride_h, stride_w, padding_h, padding_w, input, weight,
      bias, input_grad, weight_grad, bias_grad, output_grad);
}

extern "C" __global__ void conv2d_backward_reference_fp16(
    int batch_size, int input_h, int input_w, int in_channels, int out_channels,
    int kernel_size_h, int kernel_size_w, int stride_h, int stride_w,
    int padding_h, int padding_w, half *input, half *weight, half *bias,
    half *input_grad, half *weight_grad, half *bias_grad, half *output_grad) {
  return conv2d_backward_reference<half>(
      batch_size, input_h, input_w, in_channels, out_channels, kernel_size_h,
      kernel_size_w, stride_h, stride_w, padding_h, padding_w, input, weight,
      bias, input_grad, weight_grad, bias_grad, output_grad);
}